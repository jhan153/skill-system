...
- expected_inputs: request plus the minimum relevant plan slice, task list, diff, or artifact set
- expected_outputs: smallest requested response or explicitly requested document
...
3. For parallel work, assign non-overlapping lock scopes; serialize any shared-file changes. Give each task one concrete output and one validation owner.
