...
## Workflow
1. Establish the inspected scope. Record design, target app-surface, and catalog paths with versions
   or digests, plus the catalog's platform/surface scope and fallback/exception policy. When a
   product-family profile declares these,
...
3. Select or reject candidates by semantic role and behavior. Cite the selected catalog id/export/variant and nearest rejection. Use an applicable approved match unless a scoped exception or declared fallback permits another path. If none matches, keep the role `unmapped`.
...
6. Return only the assigned mapping/reuse result, decisive evidence, unresolved mappings, missing
   states, conflicts, and scoped implementation gaps. Keep proposed API changes separate.
...
- `scripts/scan_component_exports.py` inventories availability, not reuse.
