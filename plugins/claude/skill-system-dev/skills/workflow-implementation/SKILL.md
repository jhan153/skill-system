...
- context_targets:
  ...
  read_if_needed:
    ...
    - `workflow-minimal-implementation` when YAGNI, dependency restraint, or over-engineering pressure is explicit or credible
...
- Treat minimal change as an implementation-shape constraint applied only after the complete required behavior boundary is established. It must not bound investigation or turn one convenient local path into the behavior contract.
- A material regression, contradicted completion claim, repeated correction, or ownership/architecture drift invalidates the working frame and its dependent conclusions. Before another dependent plan, prompt, or edit inside this owner, reconstruct the positive objective, canonical artifacts and owners, actual production paths, materially affected consumers, and one disconfirming case.
...
3. Classify the change as local only when one production owner and path cover the outcome and no material consumer, invariant, canonical-source decision, or ownership boundary can change the deliverable.
4. For non-trivial or invalidated-frame work, establish the complete required behavior boundary before selecting a diff: positive objective, canonical artifacts and owners, actual production paths, materially affected consumers, invariants and dependency order, and one disconfirming case. For a genuinely local change, inspect its one representative caller-to-output path.
5. Select the smallest complete behavior that closes every required contract in that boundary, then change the production owner/path with the smallest coherent diff. If more than one batch is necessary, name the `delivery_shape`: `vertical_slice` for feature behavior, `migration_sequence` for a wide mechanical compatibility change, or `evidence_unit` for non-feature work. A `single_batch` does not activate the delivery-slice contract. When several batches or authorized workers are in scope, derive order and parallelism from actual dependencies, overlapping writes, and unresolved decisions. Reuse local patterns before adding a layer or dependency.
6. Observe the changed path only when agent verification is owned and allowed, using one existing verifier, direct observation, or focused smoke check that can expose the realistic failure. Add a regression test only when the user requested tests or the existing test system can cover a specific anchored regression without new framework, mock, fixture-family, or dependency work. When the user owns verification, skip this action and preserve `user-verification-needed`.
7. Inspect the diff for scope creep, accidental churn, missed materially affected consumers, stale conclusions from an invalidated frame, and policy-owning wrappers.
8. Report each material condition as evidenced, user-only, or unresolved.

## Minimum Change Discipline
This workflow stays the implementation owner. Attach `workflow-minimal-implementation` when the user asks for the smallest correct change or when the planned diff adds a dependency, abstraction, config, file, or boilerplate that current requirements do not force. The modifier receives the complete behavior boundary from this workflow and owns only the reuse ladder and over-engineering review; it must not narrow investigation or become a second owner.
