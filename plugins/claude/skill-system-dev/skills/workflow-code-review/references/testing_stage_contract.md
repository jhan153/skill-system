...
| complete test contract from an executable SUT and accepted basis | `workflow-test-design` | Core `test_design_result`, not test code or a product verdict |
...
| regression against an accepted rendered baseline | `test-visual-regression` | named pixel/framing condition evidence, not design fidelity or accessibility |
...
Before asking, Test Design supplies the blocked condition IDs, target snapshot, observations,
options, consequences, recommendation, and independent design work that can continue. Discovery
...
representative path and observable signal. It may design from an authoritative specification or
...
- data provenance and representative actual path;
...
uses Test Discovery; a missing production observation or hook returns the exact current owner.
