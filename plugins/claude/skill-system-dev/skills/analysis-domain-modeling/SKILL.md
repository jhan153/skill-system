...
5. Compare status quo with at most the material alternatives. Select names and ownership only where evidence supports them; leave unresolved business choices explicit.
6. Map the decision to concrete owner/call-site/schema changes and behavior readback. Hand implementation to `workflow-implementation` or behavior-preserving rename/extraction to `workflow-refactor-safely`.
