...
- The current task owner diagnoses correctness-only requests under a read-only boundary; `workflow-bug-fix` repairs concrete failures. `workflow-implementation` applies a selected optimization; `analysis-algorithm` owns choice without a current symptom; `analysis-codebase-map` owns Mermaid architecture maps; `research-experiment-blueprint` owns scientific benchmark design; `workflow-refactor-safely` owns preparatory behavior-preserving structure changes.
