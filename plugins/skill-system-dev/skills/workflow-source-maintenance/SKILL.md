...
Feature work remains with `workflow-implementation`; concrete repair with `workflow-bug-fix`; reachable-code restructuring with `workflow-refactor-safely`. Attach `workflow-minimal-implementation` only as YAGNI pressure. If maintenance exposes one of those needs, preserve the evidence and hand off rather than broadening this workflow.
