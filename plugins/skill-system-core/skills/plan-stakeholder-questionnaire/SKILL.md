---
name: plan-stakeholder-questionnaire
description: Create an explicitly requested Markdown question document for one stakeholder who owns missing facts or decisions. Fit the artifact to the recipient and downstream decision, verify question coverage, and keep delivery external to this skill; do not use for live requirements discovery, broad surveys, or automatic sending.
---
...
## Routing Card
- role: primary
- intent_signature: explicit local questionnaire artifact for one source-of-knowledge stakeholder
- use_when:
...
- expected_inputs: recipient role, knowledge ownership, decisions or facts needed back, downstream use, tone/language, effort or deadline constraints, and optional output path
- expected_outputs: one recipient-ready Markdown file, a coverage readback, unresolved setup assumptions, and an explicit no-delivery statement
- context_targets:
  must_read:
    - current request and supplied recipient/outcome context
    - [Stakeholder question-document template](references/stakeholder-question-document.md) before authoring
  read_if_needed:
    - only supplied notes needed to give the recipient sufficient orientation
  do_not_load_by_default:
    - full repository, unrelated requirements, contact systems, private profiles, or unstated stakeholder data
- risk_profile:
  reads: current request and explicitly supplied supporting notes
  writes: one local Markdown artifact in the authorized workspace
...
The current user owns the document setup: who will answer, why their answer is needed, what decision follows, and any delivery constraints. The stakeholder owns the missing subject matter. Ask the user only for unresolved setup that materially changes the artifact; write subject-matter questions for the stakeholder rather than asking the user to guess those answers.
...
5. **Write one artifact.** Use an exact requested path; otherwise create `stakeholder-questionnaire-<topic>.md` in the authorized workspace. If file creation is unavailable, return the complete Markdown inline and identify that no file was written.
...
Return `recipient`, `needed_back`, `artifact_path | inline_fallback`, `coverage_status`, `setup_assumptions`, and `external_delivery: not_performed`.
