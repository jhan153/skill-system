---
name: workflow-project-context-checkpoint
...
| --- | --- | --- |
| recurring interaction/execution mistake | Memory candidate | `memory-bank-update` in `candidate_mistake` mode |
| cross-session project goal, working rule, or successful practice | Memory active item | `memory-bank-update` in `durable_item` mode |
| new domain/design/algorithm/architecture knowledge or recurring repository review rule | new Knowledge category identity | `knowledge-base-record` after overlap classification |
...
| accepted durable plan/decision change | new or existing Knowledge identity | `knowledge-base-record` or `knowledge-base-update` after plan-admission and overlap classification |
