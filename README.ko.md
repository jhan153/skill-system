...
이 저장소에는 로컬 9.2.0 릴리스 컷이 포함되어 있습니다(7.3.1 번들은 아래 9.x 방향 설명대로 호환 기준선으로 유지됩니다). 주요 구성은 다음과 같습니다.
...
현재 아키텍처 라인은 `9.x — Neutral Source & Plugin Packaging`입니다. 현재 로컬 릴리스 컷은 `9.2.0 — Full Skill Diet & Bounded Evidence`이며, 미배포 9.1.2 사전 다이어트 베이스라인과 최신 배포 릴리스 `v9.1.1`을 잇습니다. `v9.2.0` 태그는 명시적으로 push하기 전까지 로컬에만 있으며, 이 컷은 설치·반영·배포하지 않았습니다.
