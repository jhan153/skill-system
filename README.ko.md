...
현재 아키텍처 라인은 `8.x — Context Compounding / Wiki Bank Architecture`입니다. 현재 구현 목표는 `8.3.0 — Bounded Loop Hardening`이며 `8.1.0 — Bounded Verification Loops` 위에 쌓입니다. `8.0.2 — Context Compounding Field Hardening`을 context-compounding field 기준선으로 유지하면서, LoopRun 컨트롤러를 session activation bridge, 단조 iteration, terminal 불변, 멱등 replay, `iterations/` 감사, precedence 반영 종료, wall-time 집행으로 하드닝합니다.
