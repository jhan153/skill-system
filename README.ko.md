...
이 저장소에는 7.2.5 기준으로 정리한 스킬 번들이 포함되어 있습니다. 주요 구성은 다음과 같습니다.
...
|   5.x | 에이전틱 워크플로와 안정화 | 계획, 실행, 검증, 보고, 검토를 서로 다른 책임으로 분리하고, 발동 조건 충돌, 스킬 간 소유권, 검증 상태, 드리프트 감사를 관리 대상으로 다루기 시작했습니다.                                                                                              |
|   6.x | 연구 라이프사이클      | 문헌 검토, 가설 수립, 실험 계획, 분석, 원고 작성, 리뷰를 하나의 연구 파이프라인으로 정리했습니다.                                                                                                                                |
