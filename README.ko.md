...
현재 아키텍처 라인은 `8.x — Context Compounding / Wiki Bank Architecture`입니다. 현재 구현 목표는 `8.1.0 — Loop Engineering Planning`입니다. `8.0.2 — Context Compounding Field Hardening`을 context-compounding field 기준선으로 유지하면서, `/goal`과 반복 agent 작업을 실행하기 전에 loop readiness, loop contract, verifier mapping, accepted loop execution 계층을 추가합니다.
...
Loop Engineering 스킬은 단순 작업을 과하게 루프화하지 않으면서, 반복 실행이 필요한 작업의 완료 조건·검증 소유권·체크포인트·중단 조건을 분리해 다룹니다. 8.1.0 계층은 진행도, 신뢰 가능한 종료, 비멱등 재시도, context poisoning, reward hacking, comprehension debt, Wiki Bank feedback candidate 같은 governance gate도 기록합니다.
...
| `workflow-loop-runner`   | 승인된 loop contract를 observe/decide/act/verify/checkpoint 배치, governance gate, verifier evidence, retry/recovery, stop condition에 따라 실행합니다. |
...
| 8.1.0 | Loop engineering planning | `/goal`과 loop 실행 전 readiness 판정, `plan-loop-term` 계약, verifier mapping, accepted loop execution 계층을 추가합니다. 성공 조건, 검증 근거, progress/stall 신호, retry/stop policy, checkpoint, idempotency note, loop governance metric, Wiki feedback candidate, 실행 인계 문구를 다룹니다. |
