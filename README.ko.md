...
Runtime assurance에는 opt-in live agent-run manifest bootstrap(`.codex/tools/init_agent_run.py`), tool/permission 운영 카탈로그(`.codex/docs/tool_policy.md`), orchestration capability contract(`.codex/docs/orchestration_capability_contract.md`)도 포함됩니다. 이들은 새 task-execution skill이 아니라 지원 계층입니다.
...
| 9.2.1 | Conditional reference disclosure | 6개 디자인 스킬에 bounded progressive disclosure를 적용하되 routing selector, evidence ceiling, fail-closed 판단은 본문에 유지합니다. `v9.2.0` 대비 본문은 638단어/4,723 UTF-8 bytes, 본문+Markdown reference 표면은 464단어/3,287 bytes 감소했습니다. fresh admission 관찰은 `design-visual-regression`과 `design-frontend`만 덮으며 나머지 4개 스킬은 universal behavior-preserved가 아니라 admission-unverified입니다. opt-in harness monitor는 verifier receipt freshness만 다루며 task result-label 권한이 없습니다. |
