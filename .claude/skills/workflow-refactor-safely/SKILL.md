...
- `analysis-codebase-design` owns an unresolved boundary; `workflow-implementation` owns feature changes; `workflow-bug-fix` owns repair; `workflow-source-maintenance` owns comments-only work in `comment_sync`; `workflow-validation` owns validation-only matrices; `workflow-minimal-implementation` attaches under YAGNI or abstraction pressure; `workflow-recovery` owns repeated post-refactor failure.
