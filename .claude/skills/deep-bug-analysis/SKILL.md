...
## Related Skills
- `deep-analysis-workflow`: router and backward-compatible entry point.
- `strict-evidence-driven-reporting-workflow`: owns execution rigor, review, and completion gates when implementation is requested.
- `strict-response-quality`: owns final formal report shape when explicitly active.
- `deep-algorithm-proposal`: use after diagnosis when the remaining question is which new approach should replace the current one.
...
- If the issue is actually approach selection, return to scheduling and use `deep-algorithm-proposal`.
