---
name: layout-constraint-translator
...
- do_not_use_when:
  - the user asks for full UI implementation; use `design-to-frontend` with this as a reference/modifier.
  - the user asks only for visual screenshot comparison; use `visual-regression-harness`.
  - the user asks only for component variant/state mapping; use `component-contract-mapper`.
...
- context_targets:
  ...
  read_if_needed:
    ...
    - design-to-frontend surface references
...
Use this skill to produce code-ready layout constraints. It can attach to `design-to-frontend`, but it should not take over full implementation.
...
6. Hand off:
   - For code changes, hand off layout rules to `design-to-frontend`.
   - For rendered proof, hand off viewport checks to `visual-regression-harness`.
...
- This skill does not choose product hierarchy; use `ui-reference-decomposer` first when hierarchy is unclear.
