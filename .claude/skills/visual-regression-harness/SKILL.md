...
- do_not_use_when:
  ...
  - the user asks for direct UI implementation; use `design-to-frontend` as primary and this skill as a supporting gate.
- expected_inputs:
...
Use this skill for visual evidence, not for implementation ownership. It can support `design-to-frontend` after a UI is rendered.
...
6. Hand off unresolved gaps:
   - Token mismatches go to `design-token-pipeline`.
   - Missing variants/states go to `component-contract-mapper`.
   - Focus/keyboard/contrast/target-size issues go to `accessibility-audit-harness`.
