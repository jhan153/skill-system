...
- context_targets:
  ...
  read_if_needed: only the exact paths named by the requested update
...
- risk_profile:
  ...
  sensitive_resources: undeclared stores and global/common Memory denied
- entry_scene: PREPARE
...
- Resolve relative paths from the manifest directory. Do not rewrite them as absolute paths merely for convenience.
...
3. Apply the smallest requested key edit. Preserve all other nodes byte-for-byte where the editing mechanism permits.
