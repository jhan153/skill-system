...
Producer: an assigned graph-mode Bug Fix after bounded attempt evidence exists. Consumer: the execution owner.
...
code_review_result repair_required + existing semantically admitted bounded-repair node -> assigned BF1 or BF2
code_review_result repair_required + first/replacement implementation requirement -> existing authorized C successor or lifecycle escalation for Plan correction; no BF attempt
code_review_result repair_required + unresolved implementation/method contract -> existing decision successor or lifecycle escalation for Plan correction; no BF attempt
...
| `skill-system-dev:workflow-bug-fix` | assigned graph-mode `bug_fix_result` and `known_bug_candidate`; standalone results stay task-local | concrete repair findings from `code_review_result` and supplied `debugging_result` evidence |
