...
| validation-only work | current task owner or named domain verifier | `workflow-rigor` only when validation itself has medium/high risk or material checker-separation requirements | changed artifact or plan/spec slice, success criteria, risk tier, available checks | broad repo audit, critical verdicts, new validation infrastructure without request |
