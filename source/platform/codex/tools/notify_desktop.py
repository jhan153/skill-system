...
def notify_linux(title: str, message: str, app: str) -> dict[str, object]:
    if shutil.which("notify-send") is None:
        return {"status": "skipped", "platform": "Linux", "reason": "notify-send not found"}
    completed = subprocess.run(
        ["notify-send", "-a", app or "codex", title, message],
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        check=False,
...
def main() -> int:
    ...
    parser.add_argument("--app", default="codex", help="codex|claude|shell|kanboard|other")
    ...
    try:
        if system == "Darwin":
            ...
            report = notify_linux(shown_title, message, args.app)
        else:
