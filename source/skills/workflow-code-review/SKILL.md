...
description: Statically review a bound implementation or diff through risk-selected, evidence-linked Mermaid models. Use before PR or integration testing to inspect state, flow, propagation, reachability, ordering, failure behavior, and optional design conformance, then return a compact Coordinator disposition. Not for mapping-only, repair, runtime validation, or Plan/Handoff topology changes.
---
...
- use_when: review bound code or a diff before PR/integration through source-derived models, with or without a design baseline.
...
- expected_inputs: snapshot/diff, review slice, code contracts/invariants, optional baseline, Known Bug exclusions, and optional node/round identity
...
- consumes: `references/core-execution-items-v1/cards/design_result.md`, `references/core-execution-items-v1/cards/implementation_result.md`, `references/core-execution-items-v1/cards/bug_fix_result.md`, `references/core-execution-items-v1/cards/known_bug_record.md`

## Ownership And Evidence Contract
- Make the implementation snapshot the primary review boundary. A supplied Core `design_result` is an optional conformance baseline: its absence disables only design comparison, never the code-derived review or disposition. Preserve its authority and unresolved fields instead of treating every mockup choice as a software invariant.
...
## Workflow
1. Bind node/round identity when supplied, implementation snapshot, review slice, exclusions, material criteria, and optional conformance sources including `design_result`.
...
4. When an authoritative baseline exists, add only the relevant conformance comparisons. Record its Core `design_result` reference when supplied. Preserve baseline ambiguity, unresolved design fields, or staleness as evidence; do not invent intended behavior or suppress intrinsic findings.
...
`design_baseline_ref` when a Core `design_result` was consumed. Keep full diagrams, matrices, and
