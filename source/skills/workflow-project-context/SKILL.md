...
- context_targets:
  must_read: exact manifest/root state, nearest repository instructions, and `.codex/docs/project_context_manifest.md`
...
Initialization and update never create store content. Bootstrap may delegate `create-store` only to `memory-bank-init` or `knowledge-base-init`; it does not populate knowledge, copy chat, initialize plan trees, or create an LLM Wiki.
