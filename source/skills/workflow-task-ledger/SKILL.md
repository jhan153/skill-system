...
See `.codex/schemas/task/task-run.schema.json` and `.codex/schemas/task/work-contract.schema.json`. TaskRun v2 stores an immutable `work_contract_ref` plus hash; form it from explicit natural-language scope rather than requiring the user to know ledger flags or YAML.
