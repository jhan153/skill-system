...
- An explicitly requested execution-ready debugging scope or runtime diagnosis through a debugger, crash artifact, dynamic diagnostic, concurrency trace, or graphics capture belongs to `workflow-runtime-debugging`; simple source/log-only diagnosis stays with the current task owner, and requested repair belongs to `workflow-bug-fix`.
