...
Resolve canonical IDs through the current installed catalog or an explicitly supplied or
repository-declared skill location. An abbreviated initial listing alone does not establish
unavailability; an unresolvable skill remains unresolved, never a guessed alias or cache path.

Workers may select available support for material subquestions discovered during the node's work.
Keep the assigned primary, accepted outcome, decision authority, write scope, and evidence contract.
Selecting support does not add a node, change an oracle, or authorize another workflow stage.
...
| `D-UI` concrete UI design artifact | Design owner | `skill-system-design:workflow-ui-design`; task-matched decomposer/layout/token/component support when material | Create the actual inspectable screen/component design from accepted requirements and return Core `design_result`. Do not write production UI code or create the implementation successor. |
...
| `V-DESIGN` scoped design evidence | Declared evidence owner | select `design-tokens`, `design-component-mapper`, `design-visual-regression`, or `design-a11y-audit` to match the Plan's assigned condition | Verify only the assigned condition. Return compact evidence/gaps without implementation edits, successor selection, automatic retry, or relabeling the Human Test phase. |
