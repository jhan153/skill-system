...
| `V` integration evidence | Coordinator | none unless the Plan names a verifier skill | Consume the compact `worker_done` body first; read one relevant artifact slice only if the decision otherwise cannot be made. Run only the verifier already named by the plan. |
