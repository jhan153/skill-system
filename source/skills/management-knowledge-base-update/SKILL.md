...
2. Confirm the change/event is accepted or source-traced, belongs to Knowledge rather than Memory/task status, and is not the same occurrence already recorded. Before any mutation below, require an explicit maintenance/reconciliation adoption covering the exact record if it is unadopted legacy; ordinary `amend`/`observe` authority is insufficient. Report a missing adoption scope and keep that record read-only; reuse adoption authority already supplied in this task. From a plan, admit only accepted/current durable decisions or rules; exclude TODOs, estimates, chronology, rejected alternatives except bounded decision history, and speculative future state.
...
5. For an adoption admitted at step 2, apply the delegated conversion: preserve the current snapshot, add the required envelope fields and `adopted_snapshot`, and state that earlier semantic history was not reconstructed. Stage adoption, any separately accepted update/event, and the index together as one bounded operation. For stable-identity meaning changes, append one semantic revision; for a new identity, create the replacement and reciprocal supersession/status links. Never silently fork or auto-merge from similarity.
