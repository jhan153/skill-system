...
| 9.2.1 conditional reference disclosure | `design-visual-regression/SKILL.md`, `design-frontend/SKILL.md`, and their existing references | Two reversible checkpoints now remove duplicated default prose without adding references. Visual regression loads its full report schema only for explicit artifact/multi-viewport work. Frontend loads product-family detail only when a profile applies and keeps token, component, visual, and accessibility owners in the selector. Cumulative main delta: -101 words/-763 bytes; owned delta: -98 words/-737 bytes. Fresh positive/negative/edge tasks preserved fail-closed missing evidence; the frontend positive also has one canonical-source `v9.2.0` comparison. | Review scoped field behavior before a third high-risk move; reject any run that selects generated copies or treats one forward sample as universal equivalence. |
