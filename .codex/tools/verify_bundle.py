...
def run_check(check: Check) -> dict[str, object]:
    ...
    if completed.returncode == 0 and stdout.startswith("SKIP"):
        status = STATUS_SKIP
...
def core_checks(root: Path) -> list[Check]:
    ...
    return [
        ...
        Check("validator_unit_tests", [py, "-m", "unittest", "discover", "-s", ".codex/tools/tests"], root),
    ]
...
def main() -> int:
    ...
    parser.add_argument("--profile", choices=["core", "integrations", "execution", "agent-output", "research"], required=True)
