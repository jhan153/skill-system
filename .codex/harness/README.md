...
| 9.2.1 result-label monitor | `SKILL_SYSTEM_HARNESS_VERSION=9.2.1` | authorize `agent-verified` from one pre-bound existing verifier whose declared subject is still current; otherwise preserve the answer and downgrade its label |
...
The 9.2.1 monitor is a small freshness and result-label monitor, not a semantic evaluator or test generator. Before `UserPromptSubmit`, the host may supply `SKILL_SYSTEM_VERIFIER_CONTRACT` with `contract_id`, one `verifier_command_hash`, `verifier_origin`, and workspace-relative `subject_refs`. Legacy oracle and negative-control fields are accepted but ignored. `PostToolUse` records matching verifier receipts, and `Stop` uses only the latest usable one. `Stop` verifies that the contract and receipt belong to the current session/turn and re-hashes the declared subject files, bounded to 16 MiB total. Read-only or unrelated later tool use does not stale the receipt; a declared subject change does.

The ordinary 9.2.1 `Stop` path starts no verifier, artifact validator, desktop notification, or Kanboard subprocess. Missing or failed evidence removes only authority to claim `agent-verified`. A strict first Stop may request one label-only reissue and explicitly forbids more tools, edits, tests, or plan expansion. An agent-authored or agent-modified verifier remains supporting evidence and is downgraded directly; the harness does not request a negative control or a larger test system. Plans should retain the selected verifier and latest decisive result only, not raw logs, receipts, retry history, or repeated passes.
