...
- do_not_use_when:
  ...
  - the user asks to create or update a normal persisted `docs/plan` artifact; use `plan-doc-workflow`.
...
- Do not create a normal `docs/plan` plan document; route that to `plan-doc-workflow`.
- Do not write to memory bank directly; output memory proposal candidates only.
- Do not perform broad evidence search; route literature-backed claims to `research-paper-evidence-search`.
...
- If plan document synchronization is requested, return to scheduling and use `plan-doc-workflow`.
