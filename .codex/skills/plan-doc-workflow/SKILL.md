...
- Continue using this workflow for the active task until user clearly closes or switches scope.

## Related Skills
- `strict-evidence-driven-reporting-workflow`: owns runtime approval/sandbox compliance, execution rigor, validation, and review when implementation starts.
- `strict-response-quality`: owns the final formal report shape when the user explicitly asks for it.
- `agent-critical-review`: use as a QA gate for plan artifacts when the user asks for review.
...
- When implementation is requested:
  ...
  - Keep logging additional findings and follow-up tasks back into the plan TODO list.
...
- If the task needs multi-phase cross-session package docs, return to scheduling and use `phase-subplan-workflow`.
...
- When clear current-task implementation approval is detected and runtime policy allows mutation:
  ...
  - Start implementation tasks in TODO order.
