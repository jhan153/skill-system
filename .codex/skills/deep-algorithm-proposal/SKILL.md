...
- `deep-bug-analysis`: use first when the current failure mode is still unclear.
- `strict-response-quality`: owns final formal report shape when explicitly active.
- `strict-evidence-driven-reporting-workflow`: owns execution rigor when the recommendation is immediately implemented.
...
- If a current failure must be diagnosed first, return to scheduling and use `deep-bug-analysis`.
