...
## Related Skills
- `strict-evidence-driven-reporting-workflow`: 수집/생성 단계가 중간 이상 위험도를 가지거나 실행 통제가 필요할 때 evidence depth, validation, review rigor를 보강합니다.
- `strict-response-quality`: 사용자가 형식적인 handoff나 감사형 요약을 원할 때 채팅 응답 형식을 정리합니다. 생성되는 리포트 파일의 장/섹션 계약은 이 스킬이 계속 소유합니다.
- `agent-critical-review`: 생성된 리포트, `findings.json`, `quality-gate-result.json`에 대해 최종 QA gate가 필요할 때 후행 검토를 담당합니다.
- `readable-diff-report`: 사용자가 실제 변경 줄 비교를 원할 때만 후처리 뷰로 사용합니다. 통합 리포트 자체를 대체하지 않습니다.
...
- `strict-response-quality`가 함께 활성화되어도 리포트 파일 구조는 바꾸지 않고, 사용자에게 보여 주는 요약 응답만 정리합니다.
- `strict-evidence-driven-reporting-workflow`가 함께 활성화되면 실행 모드, 검증, 리뷰 강도는 그 스킬을 따르되 리포트 내용 계약은 이 스킬을 유지합니다.
- `agent-critical-review`가 요청되면 리포트 생성 후 후행 QA gate로 실행합니다.
- 사용자가 diff 표현을 요구한 경우에만 `readable-diff-report`를 추가로 사용하며, integrated report findings를 prose-only 요약으로 축소하지 않습니다.
- 현재 고장 원인, 반복 버그, 알고리즘 선택 같은 point diagnosis/recommendation 요청은 이 스킬의 우선 범위가 아니며, 그런 요청은 `deep-analysis-workflow` 쪽을 우선합니다.
...
SKILL_ROOT="${SKILL_ROOT:-$CODEX_HOME/skills/codebase-intel-report}"
test -d "$SKILL_ROOT" || { echo "Set CODEX_HOME or SKILL_ROOT to codebase-intel-report" >&2; exit 1; }
```
...
- If the user asks a point-diagnosis question, return to scheduling and use `deep-analysis-workflow`.
