...
- do_not_use_when:
  - the user asks to implement code now; use `design-to-frontend`.
  - the user asks only for token normalization; use `design-token-pipeline`.
  - the user asks only for rendered screenshot comparison; use `visual-regression-harness`.
...
- If implementation is requested mid-task, hand off the decomposition to `design-to-frontend`.
