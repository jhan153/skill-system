...
## Related Skills
- `strict-response-quality`: 사용자가 형식적 보고를 원할 때 바깥 요약 문장을 정리할 수 있지만, diff block 중심 본문 구조는 이 스킬이 유지합니다.
- `agent-critical-review`: 리뷰 verdict나 finding을 먼저 도출하고, 이 스킬은 실제 변경 줄을 사람이 읽기 좋은 형태로 보여 주는 후행 표현 계층입니다.
- `codebase-intel-report`: 통합 분석/설계 리포트가 본체이고, 이 스킬은 사용자가 실제 변경 줄을 보고 싶다고 했을 때만 보조적으로 사용합니다.
...
- `strict-response-quality`가 함께 활성화되면 conclusion/action framing만 따르고, numbered diff units와 changed-lines-only 계약은 유지합니다.
...
- If the user asks for root cause, return to scheduling and use `deep-analysis-workflow`.
