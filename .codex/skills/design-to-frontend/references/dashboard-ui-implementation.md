...
Use this reference from `design-to-frontend` when the requested UI is an admin, SaaS, analytics, monitoring, reporting, or dense operational dashboard. For explicit dashboard surface work, use `$dashboard-ui-implementation`; generic design-to-code remains owned by `design-to-frontend`.
...
- Use `component-contract-mapper` for table, filter, chart, card, and state component contracts.
- Use `visual-regression-harness` for viewport-specific density and overflow evidence.
- Use `accessibility-audit-harness` for table semantics, filter labels, focus order, and chart alternatives.
