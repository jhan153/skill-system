---
name: section-based-web-implementation
...
- use_when:
  - the user explicitly invokes `section-based-web-implementation`.
  - `design-to-frontend` is implementing a section-based web page and section order, hero behavior, CTA flow, or responsive media matters.
...
- context_targets:
  ...
  read_if_needed:
    ...
    - `visual-regression-harness` for first viewport and responsive evidence
    - `accessibility-audit-harness` for headings, landmarks, CTA names, contrast, and focus order
...
Use this skill as a real section-page surface specialist. Keep `allow_implicit_invocation: false`; generic visual implementation still routes to `design-to-frontend` unless this skill is explicitly invoked or attached.
...
5. Implement or hand off:
   - If code changes are requested, follow repo patterns and `design-to-frontend` integration rules.
...
- If the design resembles marketing but the user asked for an app, hand off to `design-to-frontend` and avoid a landing-page default.
...
- It does not replace `design-to-frontend` for generic visual implementation.
