...
4. Create `knowledge_index` with `ID | Category | Title / summary | Search anchors | Related | Path | Status`, plus empty `domain`, `design`, `algorithm`, `architecture`, `code-review`, and `decisions` roots under `knowledge_root`. Use repository-compatible placeholders only when empty directories cannot persist.
...
6. Read back `knowledge_root`, `knowledge_index`, the layout, and the manifest. Do not populate inferred records.
