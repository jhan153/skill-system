...
- use_when:
  - the user explicitly invokes `dashboard-ui-implementation`.
  - `design-to-frontend` is implementing a dashboard where data density, filters, charts, tables, or async states dominate.
...
- context_targets:
  ...
  read_if_needed:
    ...
    - `component-contract-mapper` for table/filter/chart state mapping
    - `visual-regression-harness` for viewport density/overflow evidence
    - `accessibility-audit-harness` for labels, table semantics, focus, and chart alternatives
...
Use this skill as a real dashboard surface specialist. Keep `allow_implicit_invocation: false`; generic implementation still routes to `design-to-frontend` unless this skill is explicitly invoked or attached.
...
4. Map components:
   ...
   - Use `component-contract-mapper` for variants, slots, events, responsive behavior, and state coverage.
