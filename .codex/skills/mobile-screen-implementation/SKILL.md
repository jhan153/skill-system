...
- use_when:
  - the user explicitly invokes `mobile-screen-implementation`.
  - `design-to-frontend` is implementing a mobile/native screen and mobile constraints materially affect layout, state, or validation.
...
- do_not_use_when:
  - the request is a generic design-to-code task with no mobile-specific constraints; keep `design-to-frontend` primary.
...
- context_targets:
  ...
  read_if_needed:
    ...
    - `design-to-frontend` for repo integration workflow
    - `visual-regression-harness` for mobile viewport evidence
    - `accessibility-audit-harness` for keyboard/focus/label/target-size evidence
...
Use this skill as a real surface specialist, not as a roadmap placeholder. Keep `allow_implicit_invocation: false`; generic visual implementation still routes to `design-to-frontend` unless this skill is explicitly invoked or attached.
...
5. Implement or hand off:
   - If code changes are requested, follow repo patterns and the `design-to-frontend` integration rules.
...
6. Validate:
   ...
   - Use `visual-regression-harness` for screenshot/framing/overflow evidence.
   - Use `accessibility-audit-harness` for labels, focus, target size, and responsive readability.
...
## Known Limits
- This skill does not replace `design-to-frontend` for generic visual implementation.
