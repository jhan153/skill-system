...
| bug fix implementation | `workflow-bug-fix` | `analysis-bug` for unclear RCA; `workflow-recovery` for repeated same-signature failure; `workflow-validation` for check redesign; `workflow-rigor` for risky fixes | failure signal, expected behavior, repro or failing command, target files/tests | full repo report, plan package, broad redesign |
...
| performance analysis | `analysis-performance` | `workflow-implementation` only after a bottleneck and optimization are selected; `workflow-validation` for measurement plan | performance symptom, metric/workload, baseline or profile/log gap, hot path | correctness bug workflow, repo-wide report, research benchmark planning |
...
| implementation | `workflow-implementation` | `workflow-minimal-implementation` only for explicit YAGNI/minimality requests or credible over-engineering pressure; `workflow-rigor` for medium/high-risk changes; `workflow-validation` for check selection; `plan-short-term-docs` only as secondary status sync when an active plan is explicitly in scope | repo `AGENTS.md`, relevant files, active plan as input when explicitly referenced, validation | unrelated docs, plan-only completion |
| dependency upgrade | `workflow-dependency-upgrade` | `workflow-rigor` for risky upgrades; `workflow-validation` for compatibility matrix; `workflow-recovery` for repeated upgrade failure | package/runtime manifests, lockfiles, target dependency/version, usage sites, validation | broad package churn, unrelated feature work, security verdict-only review |
...
| `workflow` | `workflow-implementation` / `workflow-bug-fix` / `workflow-dependency-upgrade` / `workflow-refactor-safely` / `workflow-rigor` / `workflow-minimal-implementation` / `workflow-plan-runner` / `workflow-validation` / `workflow-recovery` by intent |
