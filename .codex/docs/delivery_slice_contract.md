...
Direct implementation may report that its scoped production change is present while lowering the task result to `unverified` or `user-verification-needed` for a missing observation. It must not call a delivery unit evidenced or complete. `workflow-plan-runner` uses the stricter state term `batch_complete`, which requires every material batch condition to have matching evidence. Review availability changes those states only when the accepted plan makes that review an exit gate.
