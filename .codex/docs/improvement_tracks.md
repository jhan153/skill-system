...
| Project context lifecycle | `project-context.yaml`, Memory Bank skills, Knowledge Base skills, `project-context-checkpoint` | Improve path resolution, compact reads, and commit/closeout classification only from explicit user reports. | User-reported misses, duplicate writes, or wrong-store decisions during real work. |
