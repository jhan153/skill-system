...
| Coordination over-trigger | `goal-workflow-orchestrator`, `multi-agent-plan-executor`, `handoff_cases.yaml`, `negative_routing_cases.yaml` | Narrow triggers to explicit DAG, handoff, lock-scope, or multi-session coordination. | Requests containing goal/목표 that should stay simple. |
| Context lifecycle drift | `spec-and-plan-curator`, `.codex/eval/runtime_usage_cases.yaml`, `.codex/eval/negative_routing_cases.yaml` | Observe whether stale plans, closed plans, and oversized instruction packets are pruned without taking over substantive task execution. | Field examples where old plans polluted context or were correctly excluded. |
| Heavy planning over-trigger | `phase-subplan-workflow`, `negative_routing_cases.yaml`, `skill_registry.md` | Require explicit package-style planning intent. | Requests containing phase, migration, rewrite, or handoff that should not create packages. |
...
| Eval result capture | `eval-harness`, `.codex/eval/usage_quality_report.template.md`, `.codex/eval/runtime_usage_cases.yaml` | Record observed primary/supporting skills, context overload, trigger misses, output shape mismatch, and friction. | Manual field review notes from representative runtime cases. |
