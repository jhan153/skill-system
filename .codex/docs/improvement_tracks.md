...
| 9.2.0 skill diet | `.codex/docs/skill_diet_protocol.md`, `.codex/eval/baselines/skill-diet-9.1.2.yaml` | All four pilot rows are checkpointed. `design-frontend` and `design-visual-regression` are rebound as one evidence-proven atomic dependency group; fresh paired reviews pass with 287 body words/2,324 bytes removed. Admission remains unverified and the checkpoint is release-ineligible. | Bootstrap an independent RSA reviewer in a separate trust-only change, then replay all retained rows through the signed release lane before shared compression or full-cohort rollout. |
