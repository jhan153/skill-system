...
- Corrected the unreleased opt-in harness 9.2.1 semantics before tagging: the reference monitor now records declared-subject receipt status only. It no longer parses or authorizes task result labels, emits validation codes or Stop reissues, feeds LoopRun, or suppresses independently enabled Agent Run, Kanboard, and notification paths. The monitor itself adds no verifier or subprocess; total Stop latency and subprocess use remain properties of the independently enabled ordinary paths.
...
- Replaced the duplicated `design-frontend` product-family checklist with a fail-closed selector to the existing `references/product-family-profile.md`, while retaining explicit ownership for token rules, catalog reuse, separate target/family verdicts, and accessibility. Its main and total owned surfaces both fall by 71 words and 391 bytes; no reference content was added.
- A canonical-source `v9.2.0`/candidate forward pair kept the same `unverified` completion decision and separated component, visual, accessibility, and critical-path gaps; the candidate additionally retained the token-rule owner and detected a catalog-digest mismatch. A family-free task omitted the product-family reference, while a consume-only token-gap task loaded it and stayed `unverified`. An earlier baseline replay that selected a generated `.claude` copy was rejected rather than counted.
