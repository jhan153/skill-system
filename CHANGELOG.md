...
- Against `v9.2.0`, the six main bodies move from 6,162 to 5,524 words (-638) and from 47,072 to 42,349 UTF-8 bytes (-4,723). Main bodies plus their Markdown references move from 10,410 to 9,946 words (-464) and from 78,661 to 75,374 bytes (-3,287), so the result is not reported as deletion achieved by merely moving prose.
- Per-skill main/main-plus-Markdown-reference word deltas are: `design-frontend` -202/-202, `design-visual-regression` -151/-130, `design-tokens` -113/-30, `design-a11y-audit` -69/-31, `design-component-mapper` -47/-47, and `design-layout-translator` -56/-24.
